...
import stereo_datasets_real as datasets
...
@torch.no_grad()
def validate_booster(model, iters=32, root="", other_params=None, mixed_prec=False):
    """ Peform validation using the Booster (TRAIN balanced) split """
    model.eval()
    aug_params = {}
    val_dataset = datasets.Booster(aug_params, root="/data6/booster", image_set="train/balanced")
    ...
    for val_id in tqdm(range(len(val_dataset))):
        ...
        imfile1 = paths[0]
        file_stem = imfile1.split('/')[-3]
        id = os.path.basename(imfile1).replace(".png","")
        output_directory = f"/data4/lzd/baseline/disp/Booster/ours_multiversion/{args.sv_name}/depth/{file_stem}"
...
def validate_fooling3d(model, iters=32, root='', mixed_prec=False, args=None, eval=False, info="", other_params=None):
    ...
    model.eval()
    aug_params = {}
    val_dataset = datasets.Fooling3DDataset(aug_params, root='/data2/Fooling3D', image_set="testing")
    ...
    for val_id in tqdm(range(len(val_dataset))):
        ...
        id = os.path.basename(imfile1).replace(".png","")
        output_directory = f"/data4/lzd/baseline/disp/Fooling3D_new/ours_multiversion/{args.sv_name}/depth/{file_stem}"
        os.makedirs(output_directory, exist_ok=True)
        writePFM(f"{output_directory}/{id}.pfm", (-flow_pr.squeeze()).detach().cpu().numpy())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sv_name', type=str, default='raftstereo', help="name of the saved model")
    ...
    if args.dataset == 'eth3d':
        ...
        res = validate_booster(model, iters=args.valid_iters, root=args.root,other_params={"fusion_iters": args.valid_fusion_iters},
                               mixed_prec=use_mixed_precision)
