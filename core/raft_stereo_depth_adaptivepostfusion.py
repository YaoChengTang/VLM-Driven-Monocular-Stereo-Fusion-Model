...
from core.update_disp import DispBasicMultiUpdateBlock, GuidedMultiUpdateBlock
...
class RAFTStereoDepthAdaptivePostFusion(nn.Module):
    def __init__(self, args):
        super(RAFTStereoDepthAdaptivePostFusion, self).__init__()
        self.args = args
        ...
        self.inp_convs_mono = nn.ModuleList([nn.Conv2d(args.hidden_dims[i]*3, args.hidden_dims[i]*3, 3, padding=3//2) for i in range(self.args.n_gru_layers)])
        self.net_convs_mono = nn.ModuleList([nn.Conv2d(args.hidden_dims[i], args.hidden_dims[i], 3, padding=3//2) for i in range(self.args.n_gru_layers)])
        self.inp_convs_stereo = nn.ModuleList([nn.Conv2d(args.hidden_dims[i]*3, args.hidden_dims[i]*3, 3, padding=3//2) for i in range(self.args.n_gru_layers)])
        self.net_convs_stereo = nn.ModuleList([nn.Conv2d(args.hidden_dims[i], args.hidden_dims[i], 3, padding=3//2) for i in range(self.args.n_gru_layers)])
        self.update_block_mono   = GuidedMultiUpdateBlock(self.args, hidden_dims=args.hidden_dims)
        self.update_block_stereo = GuidedMultiUpdateBlock(self.args, hidden_dims=args.hidden_dims)
    ...
    def forward(self, image1, image2, iters=12, disp_init=None, test_mode=False, vis_mode=False, other_params=None):
        ...
        hor_coords1_stereo = hor_coords0 - disp_refine
        hor_coords1_mono   = hor_coords0 - depth_registered
        ...
        ## Hidden state
        with autocast(enabled=self.args.mixed_precision):
            net_list_mono = [conv(x) for x, conv in zip(net_list, self.net_convs_mono)]
            inp_list_mono = [list(conv( torch.cat(x,dim=1) ).split(split_size=conv.out_channels//3, dim=1)) for x, conv in zip(inp_list, self.inp_convs_mono)]
            net_list_stereo = [conv(x) for x, conv in zip(net_list, self.net_convs_stereo)]
            inp_list_stereo = [list(conv( torch.cat(x,dim=1) ).split(split_size=conv.out_channels//3, dim=1)) for x, conv in zip(inp_list, self.inp_convs_stereo)]
        ## Iteration
        for itr in range(fusion_iters):
            hor_coords1_stereo = hor_coords1_stereo.detach()
            corr_stereo = corr_fn(hor_coords1_stereo) # index correlation volume
            disp_stereo = hor_coords1_stereo - hor_coords0

            hor_coords1_mono = hor_coords1_mono.detach()
            corr_mono = corr_fn(hor_coords1_mono) # index correlation volume
            disp_mono = hor_coords1_mono - hor_coords0

            ## Mono Guided Refinement
            with autocast(enabled=self.args.mixed_precision):
                if self.args.n_gru_layers == 3 and self.args.slow_fast_gru: # Update low-res GRU
                    net_list_stereo = self.update_block_stereo(net_list_stereo, inp_list_stereo, 
                                                               iter32=True, iter16=False, iter08=False, update=False)
                if self.args.n_gru_layers >= 2 and self.args.slow_fast_gru:# Update low-res GRU and mid-res GRU
                    net_list_stereo = self.update_block_stereo(net_list_stereo, inp_list_stereo, 
                                                               iter32=self.args.n_gru_layers==3, iter16=True, iter08=False, update=False)
                net_list_stereo, up_mask_stereo, delta_stereo = self.update_block_stereo(net_list_stereo, inp_list_stereo, corr_mono, disp_mono, disp_stereo, 
                                                                                iter32=self.args.n_gru_layers==3, iter16=self.args.n_gru_layers>=2)
            # F(t+1) = F(t) + \Delta(t)
            hor_coords1_stereo = hor_coords1_stereo + delta_stereo

            ## Stereo Guided Alignment
            with autocast(enabled=self.args.mixed_precision):
                if self.args.n_gru_layers == 3 and self.args.slow_fast_gru: # Update low-res GRU
                    net_list_mono = self.update_block_mono(net_list_mono, inp_list_mono, 
                                                               iter32=True, iter16=False, iter08=False, update=False)
                if self.args.n_gru_layers >= 2 and self.args.slow_fast_gru:# Update low-res GRU and mid-res GRU
                    net_list_mono = self.update_block_mono(net_list_mono, inp_list_mono, 
                                                               iter32=self.args.n_gru_layers==3, iter16=True, iter08=False, update=False)
                net_list_mono, up_mask_mono, delta_mono = self.update_block_mono(net_list_mono, inp_list_mono, corr_stereo, disp_stereo, disp_mono, 
                                                                                iter32=self.args.n_gru_layers==3, iter16=self.args.n_gru_layers>=2)
            # F(t+1) = F(t) + \Delta(t)
            hor_coords1_mono = hor_coords1_mono + delta_mono
            ...
            # upsample predictions
            disp_up_mono = self.upsample_disp(hor_coords1_mono - hor_coords0, up_mask_mono)
            disp_predictions.append(disp_up_mono)
            disp_up_stereo = self.upsample_disp(hor_coords1_stereo - hor_coords0, up_mask_stereo)
            disp_predictions.append(disp_up_stereo)



        if test_mode:
            return hor_coords1_stereo - hor_coords0, disp_up_stereo
