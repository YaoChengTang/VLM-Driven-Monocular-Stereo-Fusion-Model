...
from core.raft_stereo_depth_postfusion_noctx import RAFTStereoDepthPostFusionNoCTX
...
def get_model_ddp(args):
    if args.model_name.lower() == "raftstereo":
        ...
        model = nn.SyncBatchNorm.convert_sync_batchnorm(RAFTStereoDepthAdaptiveSingle(args))
    elif args.model_name.lower() == "RAFTStereoDepthPostFusionNoCTX".lower():
        model = nn.SyncBatchNorm.convert_sync_batchnorm(RAFTStereoDepthPostFusionNoCTX(args))
    else :
