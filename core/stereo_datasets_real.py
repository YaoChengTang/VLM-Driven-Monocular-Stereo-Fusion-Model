...
class Fooling3DDataset(StereoDataset):
    ...
    def _add_mono(self):
        ...
        if self.image_set=="training":
            ...
            with open(os.path.join(self.root, 'real/testing_enter.pkl'), 'rb') as f:
                data = pickle.load(f)
            
            self.extra_info["mask"] = []
            for key, frame_dict in data.items():
                left_image_path  = os.path.join(self.root, "real/test", frame_dict["left"])
                right_image_path = os.path.join(self.root, "real/test", frame_dict["right"])
                disp_image_path  = os.path.join(self.root, "real/test", frame_dict["disp"])
                mask_image_path  = os.path.join(self.root, "real/test", frame_dict["mask"])
