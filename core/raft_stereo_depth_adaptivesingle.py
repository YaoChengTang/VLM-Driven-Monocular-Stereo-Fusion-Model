...
class RAFTStereoDepthAdaptiveSingle(nn.Module):
    def __init__(self, args):
        super(RAFTStereoDepthAdaptiveSingle, self).__init__()
        self.args = args
        ...
        self.update_block_stereo = GuidedMultiUpdateBlock(self.args, hidden_dims=args.hidden_dims, single=True)
    ...
    def forward(self, image1, image2, iters=12, disp_init=None, test_mode=False, vis_mode=False, other_params=None):
        ...
        for itr in range(fusion_iters):
            ...
            with autocast(enabled=self.args.mixed_precision):
                ...
                net_list_stereo, up_mask_stereo, delta_stereo = self.update_block_stereo(net_list_stereo, inp_list_stereo, 
                                                                                torch.cat([corr_mono, corr_stereo], dim=1), disp_mono, disp_stereo, 
